...
def create_lccsr(num_entities, num_edges, edges, edge_timestamps, directed=False, load=False, load_path=None):
    if load:
        if load_path is None:
            raise ValueError("Path to load adjacency matrix must be specified")
        if os.path.exists(load_path):
            edge_lccsr = LeftContiguousCSR.load(load_path)
            return edge_lccsr
    ...
    edge_csr_timestamps = np.zeros((total_num_edges,), dtype=np.int64)
    ...
    if load:
        edge_lccsr.save(load_path)

    return edge_lccsr, degrees, indegrees, outdegrees


def convert_to_compact_adj(graph: InteractionGraph, load_edge_lccsr=False, edge_lccsr_path=None):
    num_entities = len(graph.user_data) + len(graph.item_data)
    num_edges = len(graph.train_edges)
    edge_lccsr, degrees, indegrees, outdegrees = create_lccsr(num_entities, num_edges, graph.train_edges, graph.interaction_time_stamps, load=load_edge_lccsr, load_path=edge_lccsr_path)
